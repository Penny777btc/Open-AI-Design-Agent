...
def make_contact_shadow(cutout_png: bytes, frame_w: int, frame_h: int, *, horizon: float = 0.62,
    ...
    c = _open_trim(cutout_png)
    cw, ch = c.size
    if cw < 1 or ch < 1:
        return cutout_png
    x, y, tw, th = _grounded_box(frame_w, frame_h, cw, ch, horizon, h_ratio)
    base_cx = x + tw / 2
    base_cy = y + th  # 底边 = horizon 线
    ew = tw * 0.92
    eh = max(8.0, tw * 0.20)

    az = str((light or {}).get("azimuth", "upper-left")).lower()
    dx = ew * 0.18 if "left" in az else (-ew * 0.18 if "right" in az else 0.0)
    try:
        elev = max(10.0, min(80.0, float((light or {}).get("elevation", 35))))
    except (TypeError, ValueError):
        elev = 35.0
    dx *= max(0.6, min(2.0, 1.0 / math.tan(math.radians(elev))))  # 光越低 → 阴影越长
    cx = base_cx + dx

    shadow = Image.new("RGBA", (frame_w, frame_h), (0, 0, 0, 0))
    ImageDraw.Draw(shadow).ellipse(
        [cx - ew / 2, base_cy - eh / 2, cx + ew / 2, base_cy + eh / 2], fill=(0, 0, 0, 150),
    )
    shadow = shadow.filter(ImageFilter.GaussianBlur(radius=max(4.0, ew * 0.06)))
