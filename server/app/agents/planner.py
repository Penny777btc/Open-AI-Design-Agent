...
- 【人物模式 person_mode】含真人的 edit_image 还须在 args 里加 "person_mode"，二选一：
  · 默认 "lock"（锁人物合成）——绝大多数情况用它：系统用人像专用模型抠出人物原始像素(零变形、
    柔边、只抠人不带桌子餐盘) + AI 只生成背景/排版(明确 NO PERSON、下中部留位) + 柔边合成。人脸/
    身材 100% 不动、不拉伸、无硬边贴纸感。凡「换背景/加海报排版/做封面/合成到场景/加文字」一律 "lock"。
  · 仅当用户**明确要求把人物本身画进画面/风格化/画成插画/改造人物外观**（如"把他画成动漫风"
    "把人物融进油画质感"）时才用 "fuse"（扩散模型整图重绘人物）。拿不准就用 "lock"。
